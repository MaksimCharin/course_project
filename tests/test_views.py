...
from src.views import get_greeting, get_spending_data, generate_report
...
def transactions():
    ...
    data = {
        'Дата операции': ['2023-10-01 10:00:00', '2023-10-02 12:00:00', '2023-10-03 14:00:00'],
...
def test_get_spending_data(transactions):
    ...
    expected_data = {
        '1234567890123456': {
            ...
            'cashback': 35,
            'top_transactions': [
                {'Дата операции': '2023-10-02 12:00:00', 'Категория': 'Транспорт', 'Сумма платежа': 2000},
                {'Дата операции': '2023-10-01 10:00:00', 'Категория': 'Еда', 'Сумма платежа': 1500}
            ]
        },
        '9876543210987654': {
            'last_4_digits': '7654',
            ...
            'cashback': 30,
            'top_transactions': [
                {'Дата операции': '2023-10-03 14:00:00', 'Категория': 'Еда', 'Сумма платежа': 3000}
            ]
        }
    }
...
def test_generate_report_success(mock_greeting, mock_stock_prices, mock_currency_rates, mock_spending_data, mock_open,
                                 mock_read_excel):
...
@patch('src.views.get_stock_prices')
@patch('src.views.get_greeting')
def test_generate_report_invalid_date_format(mock_greeting, mock_stock_prices, mock_currency_rates, mock_spending_data,
                                             mock_open, mock_read_excel):
    ...
    # Проверка обработки ошибки с неверным форматом даты
    with unittest.TestCase.assertRaises(self, ValueError):
        result = generate_report("invalid_date")
